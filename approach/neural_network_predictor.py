...
class NeuralNetworkPredictor(PredictionModel):
    ...
    def update_model(self, X_train: pd.Series, y_train: float) -> None:
        ...
        When ``use_online_grid`` is ``False`` the sample is appended to the
        stored training data, scalers are refit on the full dataset, and the
        model is retrained via ``_selectBestModel``. If ``use_online_grid`` is
        ``True`` the update is delegated to ``_update_online_grid``.
        """
        if self.use_online_grid:
    ...
    def _selectBestModel(self, X_train, y_train):
        ...
        :param y_train: Training labels.
        """
        # Reinitialize scalers on the provided data
